...
function megasena(){
    // Esta funcção retorna um número aleatório entre 1 e 60
...
// 6 números de um jogo megasena
console.log(megasena(), megasena(), megasena(), megasena(), megasena(), megasena())

// Em arrow functions sem parâmetros, é necessário deixar um par vazio de parêntesis
...
//Funçãocom mais de uma linha de código no corpo
...
// Em arrow functions com mais de uma linha no corpo, as chaves retoram e 
//também é necessário voltar a usar a palavra return para retornar um valor

const domaTudo2 = (... nums) => {
    let soma = 0
    //O parâmetro de resto, dentro de uma função, é interpretado como um vetor
    for(let n of nums) soma += n
    return soma
...
//CONCLUSÃO: Arrow functions são ideiais para quando uma função tem apenas uma 
